...
$html .= "<div class=\"attContent\">\n";
$html .= "<h3 class=\"at_title\">{$this->title}</h3>\n";
...
$html .= "<div class=\"attContent-inner\">\n";
...
for ($i=0, $n=count($attachments); $i < $n; $i++) {
	...
	if ( $this->show_file_links ) {
		...
		$html .= "<span><a class=\"at_icon\" href=\"$url\"$target title=\"$tooltip\">";
		...
		if ( ($attachment->uri_type == 'url') && $this->superimpose_link_icons ) {
			...
			$html .= "</a></span>";
			$html .= "<h4><a class=\"at_url\" href=\"$url\"$target title=\"$tooltip\">$filename</a></h4>";
		}
	else {
		$tooltip = JText::sprintf('ATTACH_DOWNLOAD_THIS_FILE_S', $actual_filename);
		$html .= JHtml::image('com_attachments/file_icons/'.$icon, $tooltip, null, true);
	...
	if ( $this->show_description ) {
		...
		if ( JString::strlen($description) == 0)
			$description = '&nbsp;';
		if ( $this->show_column_titles )
			$html .= "<div class=\"at_description\"><p>$description</p></div>";
		else
			$html .= "<div class=\"at_description\"><p>[$description]</p></div>";
		}
		$html .= "<div class=\"at_baixar\"><a class=\"at_url btn btn-warning\" href=\"$url\"$target title=\"$tooltip\">Download</a></div>";
	...
	if ( $this->show_user_field_1 ) {
		...
		if ( JString::strlen($user_field) == 0 )
			$user_field = '&nbsp;';
		if ( $this->show_column_titles )
			$html .= "<div class=\"at_user_field\">" . $user_field . "</div>";
		else
			$html .= "<div class=\"at_user_field\">[" . $user_field . "]</div>";
		}
	if ( $this->show_user_field_2 ) {
	...
	if ( $this->show_creator_name ) {
		$html .= "<div class=\"at_creator_name\">{$attachment->creator_name}</div>";
		}
	...
	if ( $this->show_file_size ) {
		...
		$html .= '<div class="at_file_size">' . $file_size_str . '</div>';
		}
	...
	if ( $this->secure && $this->show_downloads ) {
		...
		$html .= '<div class="at_downloads">'. $num_downloads.$label.'</div>';
		}

	// Add the created and modification date (maybe)
	if ( $this->show_created_date ) {
		$html .= "<div class=\"at_created_date\">$created</div>";
		}
	if ( $this->show_modified_date ) {
		$html .= "<div class=\"at_mod_date\">$last_modified</div>";
		}
	...
	if ( $this->some_attachments_modifiable && $this->allow_edit ) {
		$html .= "<div class=\"at_edit\">$update_link $delete_link</div>";
		}
