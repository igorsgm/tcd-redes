 ...
 * @since  1.6
 */
class UsersControllerCustomReset extends UsersController
{
	/**
	...
	public function request()
	{
		...
		if (empty($data['email']) && !empty($data['cpf'])) {
			$username = preg_replace('/\D/', '', $data['cpf']);
			$data['email'] = JFactory::getUser(JUserHelper::getUserId($username))->email;
		}

		$app->setUserState('com_users.reset.email', $data['email']);
	...
	public function confirm()
	{
		...
		if (!empty($data['username'])) {
			$data['username'] = preg_replace('/\D/', '', $data['username']);
		}

		$app->setUserState('com_users.reset.email', null);
