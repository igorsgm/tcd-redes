...
<?php
	...
	$getImage = JLayoutHelper::render('joomla.content.intro_image', $this->item);

	if (empty($images->image_intro)) {
		$getImage = JLayoutHelper::render('joomla.content.full_image', $this->item);
	}
...
<div class="entry-header<?php echo $tpl_params->get('show_post_format') ? ' has-post-format': ''; ?>">
	...
	<p>
		<?php
			...
			// informa o código que será cortado do introtext, referente ao attachments
 			$cut = '<div class=';
 			// faz o corte de tudo que vem após o $cut
			$textolimpo = substr($this->item->introtext, 0, strpos($this->item->introtext, $cut));
			// limitando os caracteres do introtext sem tags, de acordo com o limite
			 $introtxt = strip_tags($textolimpo);
			 $intro = substr($introtxt, 0, 140);
			 echo $intro;
			 if(strlen($introtxt) > 140) {
			 	echo "...";
			 }
		?>
	</p>
...
<!-- Pega o autor do item  -->
	<span class="author"><?php echo json_decode($this->item->metadata)->author; ?></span>
