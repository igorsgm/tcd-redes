 ...
 * @since  1.6
 */
class OuvidoriaModelSolicitanteForm extends JModelForm
{
	private $item = null;
	...
	public function &getData($id = null)
	{
		if ($this->item === null) {
			...
			if ($table !== false && $table->load($id)) {
				...
				if ($this->item->is_associado = !empty($this->item->id_associado)) {
					$this->item->amatra = ThomisticusHelperModel::select('#__associados', 'amatra', ['id' => $this->item->id_associado], 'Result');
				}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_ouvidoria.solicitante', 'solicitanteform', array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
		);
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('solicitante.id');
	...
	public function delete($pk)
	{
		...
		if (empty($pk)) {
			$pk = (int)$this->getState('solicitante.id');
		}
