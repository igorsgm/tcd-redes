 ...
 * @since  1.6
 */
class OuvidoriaModelSolicitacaologForm extends JModelForm
{
	private $item = null;
	...
	{
		// Get the form.
		$form = $this->loadForm('com_ouvidoria.solicitacaolog', 'solicitacaologform', array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
		);
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('solicitacaolog.id');
	...
	public function delete($pk)
	{
		...
		if (empty($pk))
		{
			$pk = (int) $this->getState('solicitacaolog.id');
		}
