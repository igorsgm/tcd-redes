 ...
 * @since  1.6
 */
class OuvidoriaModelComentarioForm extends JModelForm
{
	private $item = null;
	...
	protected function populateState()
	{
		...
		if (JFactory::getApplication()->input->get('layout') == 'edit') {
			$id = JFactory::getApplication()->getUserState('com_ouvidoria.edit.comentario.id');
		} else {
			$id = JFactory::getApplication()->input->get('id');
			JFactory::getApplication()->setUserState('com_ouvidoria.edit.comentario.id', $id);
		}

		$this->setState('comentario.id', $id);
		...
		if (isset($params_array['item_id'])) {
			$this->setState('comentario.id', $params_array['item_id']);
		}
	...
	{
		if ($this->item === null || !empty($id)) {
			$this->item = false;

			if (empty($id)) {
				$id = $this->getState('comentario.id');
			}
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int)$this->getState('comentario.id');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_ouvidoria.comentario', 'comentarioform', array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
		);
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('comentario.id');
	...
	public function delete($pk)
	{
		...
		if (empty($pk)) {
			$pk = (int)$this->getState('comentario.id');
		}
