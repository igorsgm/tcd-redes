 ...
 * @since  1.6
 */
class OuvidoriaControllerComentario extends JControllerLegacy
{
	/**
	...
	public function edit()
	{
		...
		// Get the model.
		$model = $this->getModel('Comentario', 'OuvidoriaModel');
	...
	public function publish()
	{
		...
		if ($user->authorise('core.edit', 'com_ouvidoria') || $user->authorise('core.edit.state', 'com_ouvidoria')) {
			...
			if (!$item) {
				...
				$this->setRedirect(JRoute::_('index.php?option=com_ouvidoria&view=comentarios', false));
			} else {
	...
	public function remove()
	{
		...
		if ($user->authorise('core.delete', 'com_ouvidoria')) {
			...
			if ($return === false) {
				...
				$app->redirect(JRoute::_('index.php?option=com_ouvidoria&view=comentarios', false));
			}
