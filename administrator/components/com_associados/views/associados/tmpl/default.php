...
$document->addStyleSheet(JUri::root() . 'administrator/components/com_associados/assets/css/associados.css');
$document->addStyleSheet(JUri::root() . 'media/com_associados/css/list.css');

$user      = JFactory::getUser();
$userId    = $user->get('id');
$listOrder = $this->state->get('list.ordering');
$listDirn  = $this->state->get('list.direction');
$canOrder  = $user->authorise('core.edit.state', 'com_associados');
$saveOrder = $listOrder == 'a.`ordering`';

$app = JFactory::getApplication();
$qtdJaLogaram = $app->getUserState('com_associados.qtdJaLogaram');
$qtdJaLogaramAfterReset = $app->getUserState('com_associados.qtdJaLogaramAfterReset');

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_associados&task=associados.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'associadoList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
    ...
    <div id="j-main-container" class="span10">
        ...
        <div id="j-main-container">
            ...
            <div id="filter-bar" class="btn-toolbar">
                ...
                <a href="index.php?option=com_associados&view=associados&tpl=excelassociados&tmpl=component"
                   class="btn btn-info">
                    <i class="fa fa-download"></i> Gerar Excel <b>Associados</b>
                </a>
            </div>

            <div class="alert alert-info">
                <div><b>(Pesquisa) Quantidade de Associados:</b> <u><?php echo $this->pagination->total; ?></u>
                    associado(s)
                </div>
                <hr style="margin: 5px 0;">
                <div><b>(Total) Já logaram no Espaço do
                        Associado: </b><u><?php echo $qtdJaLogaram; ?></u>
                    associado(s)
                    -- Destes, <u><?php echo $qtdJaLogaramAfterReset; ?></u> <b>após reenvio de senha</b>.
                </div>
				<?php $naologaram = intval($this->pagination->total) - $qtdJaLogaram; ?>
				<?php if ($naologaram >= 0): ?>
                    <div><b>(Total) Ainda <u>não</u> logaram
                        </b><?php echo(intval($this->pagination->total) - $qtdJaLogaram); ?> associado(s)
                    </div>
				<?php endif; ?>
            </div>

            <div class="clearfix"></div>
            <table class="table table-striped" id="associadoList">
                <thead>
                <tr>
                    ...
                    <th class='left'>
						<?php echo JHtml::_('grid.sort', 'COM_ASSOCIADOS_ASSOCIADOS_USER_ID', 'a.`user_id`', $listDirn, $listOrder); ?>
                    </th>
                    <th class='left'>
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canCreate = $user->authorise('core.create', 'com_associados');
					$canEdit = $user->authorise('core.edit', 'com_associados');
                    ...
                    <tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
                            <td class="center">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'associados.', $canChange, 'cb'); ?>
                            </td>
						<?php endif; ?>
                        ...
                        <td>
							<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'associados.', $canCheckin); ?>
							<?php endif; ?>
							<?php if ($canEdit) : ?>
                                <a href="<?php echo JRoute::_('index.php?option=com_associados&task=associado.edit&id=' . (int) $item->id); ?>">
