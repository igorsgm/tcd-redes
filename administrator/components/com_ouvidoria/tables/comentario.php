...
/**
 * comentario Table class
 *
 * @since  1.6
 */
class OuvidoriaTablecomentario extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'OuvidoriaTablecomentario', array('typeAlias' => 'com_ouvidoria.comentario'));
		parent::__construct('#__ouvidoria_comentarios', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_ouvidoria.comentario.' . $array['id'])) {
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_ouvidoria/access.xml',
				"/access/section[@name='comentario']/"
			);
			$default_actions = JAccess::getAssetRules('com_ouvidoria.comentario.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_ouvidoria.comentario.' . (int)$this->$k;
	}
