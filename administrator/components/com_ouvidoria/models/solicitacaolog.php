 ...
 * @since  1.6
 */
class OuvidoriaModelSolicitacaolog extends JModelAdmin
{
	/**
	 ...
	 * @since   3.2
	 */
	public $typeAlias = 'com_ouvidoria.solicitacaolog';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_ouvidoria.solicitacaolog', 'solicitacaolog',
			array(
				'control'   => 'jform',
				'load_data' => $loadData
			)
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ouvidoria.edit.solicitacaolog.data', array());
	...
	/**
	 * Method to duplicate an Solicitacaolog
	 *
	...
	public function duplicate(&$pks)
	{
		...
		foreach ($pks as $pk) {
			if ($table->load($pk, true)) {
				...
				if (!empty($table->id_interacao)) {
					if (is_array($table->id_interacao)) {
						$table->id_interacao = implode(',', $table->id_interacao);
					}
				} else {
					$table->id_interacao = '';
				}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__ouvidoria_solicitacoes_logs');
				$max             = $db->loadResult();
