...
/**
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Load the parameters.
		$params = JComponentHelper::getParams('com_dispositivos');
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.tipo LIKE ' . $search . '  OR  a.modelo LIKE ' . $search . '  OR  a.sistema_operacional LIKE ' . $search . ' )');
			}
		}
		...
		if ($filter_tipo !== null && !empty($filter_tipo))
		{
		...
		foreach ($items as $oneItem)
		{
			...
			if (isset($oneItem->tipo))
			{
				...
				$query
					...
					->where('FIND_IN_SET(' . $db->quoteName('id') . ', ' . $db->quote($oneItem->tipo) . ')');

				$db->setQuery($query);
				$result = $db->loadColumn();

				$oneItem->tipo = !empty($result) ? implode(', ', $result) : '';
			}
		}
