...
class senderprofileViewsenderprofile extends acysmsView{
	...
	function form(){
		...
		$script .= 'if(window.document.getElementById("datasenderprofilesenderprofile_gateway").value == "acygateway"){
			...
			if(!filter.test(window.document.getElementById("senderprofile_sendername").value)){
				alert("The sender name is incorrect: you must use a phone number including the country code or a word of 4 to 11 characters");
				return false;
			}
		}';
		if(version_compare(JVERSION, '1.6.0', '<')){
