...
class AgendaModelAgendas extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
		...
		//Filtering data_inicio
		$this->setState('filter.data_inicio', $app->getUserStateFromRequest($this->context.'.filter.data_inicio', 'filter_data_inicio', '', 'string'));


        // Load the parameters.
        $params = JComponentHelper::getParams('com_agenda');
    ...
    protected function getListQuery() {
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.categoria LIKE '.$search.'  OR  a.nome LIKE '.$search.'  OR  a.local LIKE '.$search.'  OR  a.data_inicio LIKE '.$search.' )');
            }
        }
