	...
	window.ImageManager = {
		...
		initialize: function () {
			...
			this.fields = {
				...
				'credit': doc.getElementById("f_credit"),
		...
		onok: function () {
			var tag = '',
				...
				credit = this.fields.credit.value,
			...
			if (url) {
				...
				if (caption || credit) {
					...
					if (credit) {
						// spancredit = '<span class="itemImageCredits">' + credit + '</span>';
						arialabel = ' aria-label="' + credit + '" ';
					}
					...
					figstyle = ' style="width:' + this.getImageWidth(juri_base + url) + 'px;"';


					tag = '<figure' + arialabel + figclass + figstyle + '>' + spancredit + tag + '<figcaption' + captionclass + '>' + caption + '</figcaption></figure>';
				}
			}
		...
		onfilepost: function (post, arkimage, folder) {

			var numFiles = document.getElementById("upload-file");
			var textFiles = jQuery("#f_text").val();
			var regxFiles = new RegExp("fakepath");
			var res = regxFiles.test(post);
			if (numFiles.files.length == 1 && (typeof textFiles == 'undefined' || textFiles == '') && arkimage != 1 ) {	
				if (folder != '') {
					sessionStorage['img_path'] = res ? post.replace(/C:\\fakepath\\*/g, "images/" + folder + "/") : "images/"+ folder + "/" + post;
				}else{
					sessionStorage['img_path'] = res ? post.replace(/C:\\fakepath\\*/g, "images/") : "images/" + post;
				}
			}
		},

		/**
		 * Called to get image width inside onok method and used to create figure style
		 * @param   string  url     image's URL
		 * @returns integer         image's width
		 */
