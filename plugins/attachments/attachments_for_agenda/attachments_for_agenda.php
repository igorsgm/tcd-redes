 ...
 * @since	 3.0
 */
class AttachmentsPlugin_Com_Agenda extends AttachmentsPlugin
{
	/**
	 * Constructor
	 *
	...
	public function __construct(&$subject, $config = array())
	{
		...
		$this->_name		  = 'attachments_for_agenda';

		// Set basic attachments defaults
		$this->parent_type	  = 'com_agenda';
		$this->default_entity = 'agenda';

		// Add the information about the default entity (article)
		$this->entities[]					 = 'agenda';
		$this->entity_name['agenda']		 = 'agenda';
		$this->entity_name['default']		 = 'agenda';
		$this->entity_table['agenda']		 = 'content';
		$this->entity_id_field['agenda']	 = 'id';
		$this->entity_title_field['agenda'] = 'title';
	...
	public function determineParentEntity(&$parent)
	{
		...
		if (($view == 'category') && (get_class($parent) == 'JTableAgenda'))
		{
			return 'category';
		}
	...
	public function getSelectEntityURL($parent_entity = 'default')
	{
		...
		switch ($parent_entity)
		{
			...
			default:
				return "index.php?option=com_agenda&amp;view=agendas&amp;layout=modal&amp;tmpl=component&amp;function=jSelectAgenda";
		}
	}
	...
	public function getEntityItems($parent_entity = 'default', $filter = '')
	{
		...
		$query->where('extension=' . $db->quote('com_agenda'));
	...
	public function getParentCreatorId($parent_id, $parent_entity = 'default')
	{
		...
		switch ($parent_entity)
		{
			...
			default: // Article
				$query->select('created_by')->from('#__com_agenda')->where('id = ' . (int) $parent_id);
				$db->setQuery($query, 0, 1);
				$result = $db->loadResult();
				if ($db->getErrorNum())
				{
	...
	public function getEntityViewURL($parent_id, $parent_entity = 'default')
	{
		...
		switch ($parent_entity)
		{
			...
			case 'category':
				return $base_url . 'index.php?option=com_agenda&view=category&id=' . $parent_id;
				break;

			default:
				return $base_url . 'index.php?option=com_agenda&view=agenda&id=' . $parent_id;
		}
	}
	...
	public function newParent(&$attachment)
	{
		...
		if ($attachment->parent_entity == 'agenda') {
	...
	public function isParentPublished($parent_id, $parent_entity = 'default')
	{
		...
		switch ($parent_entity)
		{
			...
			default:
				...
				// Check for articles
				$query = $db->getQuery(true);
				$query->select('state, publish_up, publish_down')->from('#__com_agenda');
	...
	public function isParentArchived($parent_id, $parent_entity = 'default')
	{
		...
		switch ($parent_entity)
		{
			...
			default:
				...
				$db	   = JFactory::getDBO();
				$query = $db->getQuery(true);
				$query->select('state')->from('#__com_agenda')->where(' id = ' . (int) $parent_id);
				$db->setQuery($query, 0, 1);
				$article = $db->loadObject();
				if ($db->getErrorNum())
				{
	...
	public function getParentPublishedFilter($parent_state, $filter_entity)
	{
		...
		if ($parent_state == 'PUBLISHED')
		{
			...
			if (($filter_entity == 'ALL') || ($filter_entity == 'AGENDA'))
			{
				$now	  = JFactory::getDate()->toSql();
				$nullDate = $db->getNullDate();
				$where[]  = "EXISTS (SELECT * FROM #__com_agenda AS c1 " .
	...
	public function attachmentsHiddenForParent(&$parent, $parent_id, $parent_entity)
	{
		...
		else
		{
			...
			$query = $db->getQuery(true);
			$query->select('created_by, catid')->from('#__com_agenda')->where('id = ' . (int) $parent_id);
			$db->setQuery($query);
	...
	public function userMayEditAttachment(&$attachment, $user_id = null)
	{
		...
		$user = JFactory::getUser($user_id);
		if ($user->authorise('com_agenda', 'edit', 'content', 'all'))
		{
			return true;
		}
		...
		switch ($attachment->parent_entity)
		{
			...
			default:
				...
				if ($user->authorise('attachments.edit.ownparent', 'com_attachments'))
				{
					$article_creator_id = $this->getParentCreatorId($attachment->parent_id, 'agenda');
	...
	public function userMayChangeAttachmentState($parent_id, $parent_entity, $attachment_creator_id, $user_id = null)
	{
		...
		$user = JFactory::getUser($user_id);
		if ($user->authorise('com_agenda', 'edit', 'agenda', 'all'))
		{
			return true;
		}
	...
	{
		return ($layout =='edit') && (($view == 'form') || ($view == 'agenda') || ($view == 'category'));
	}
	...
	{
		return array_merge(parent::knownFroms(), Array('frontpage', 'featured', 'agenda', 'category', 'details'));
	}
